...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--tokens', help='type of the output tokens, use p for raw POS, t for POS with tags and l for lemmas (default)', type=str, choices='plt', default='l')
    args = parser.parse_args()
    with concraft.Server() as server:
        client = concraft.Client(server.get_port())
        mapping = {'l': client.to_lemmas, 't': client.to_pos_tags, 'p': client.to_pos}
