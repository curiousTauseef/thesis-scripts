...
name = rep("unigrams", nrow(unigrams_downsampled))
unigrams_col = data.frame(unigrams_downsampled, name)
colnames(unigrams_col) = c("token", "count", "index", "name")

name = rep("bigrams", nrow(bigrams_downsampled))
bigrams_col = data.frame(bigrams_downsampled, name)
colnames(bigrams_col) = c("token", "count", "index", "name")

name = rep("trigrams", nrow(trigrams_downsampled))
trigrams_col = data.frame(trigrams_downsampled, name)
colnames(trigrams_col) = c("token", "count", "index", "name")

data = rbind(unigrams_col, bigrams_col, trigrams_col)
...
#theme(axis.title.x=element_text(margin=margin(20,0,0,0)))
#
#common_plot = ggplot(data, aes(x=log(index), y=log(count), colour=name)) +
